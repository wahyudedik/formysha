    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-3xl p-4 sm:p-6 lg:p-8">
                <div class="mb-6">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('Profil Anak') }}</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Isi informasi dasar buah hati Anda.') }}</p>
                </div>

                <form method="POST" action="{{ route('children.store') }}" enctype="multipart/form-data" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea id="bio" name="bio" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-softPink-300 focus:ring-softPink-200 rounded-xl shadow-sm transition" placeholder="Ceritakan sedikit tentang buah hati Anda...">{{ old('bio') }}</textarea>
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-4">
                        ...
                        <a href="{{ route('children.index') }}" class="inline-flex items-center justify-center px-6 py-2.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-xl text-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition min-h-[44px]">
                            {{ __('Batal') }}
                        </a>
                    </div>
                </form>
