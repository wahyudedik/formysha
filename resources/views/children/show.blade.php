    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
                <a href="{{ route('children.index') }}" class="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 transition">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-3xl">
                <div class="bg-gradient-to-br from-softPink-50 via-cream-50 to-lavender-50 dark:from-softPink-950/30 dark:via-gray-800 dark:to-lavender-950/30 p-6 sm:p-8">
                    <div class="flex items-center gap-6">
                        ...
                        @else
                            <div class="w-24 h-24 rounded-2xl {{ $child->gender === 'female' ? 'bg-softPink-100 dark:bg-softPink-950/30' : 'bg-skyBlue-100 dark:bg-skyBlue-950/30' }} flex items-center justify-center text-4xl shadow-soft-md">
                                {{ $child->gender === 'female' ? '👧' : '👦' }}
                            </div>
                        @endif
                        <div>
                            <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ $child->name }}</h3>
                            @if ($child->nickname)
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-3xl p-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $child->date_of_birth->format('d M Y') }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-500 dark:text-gray-400">Usia</span>
                            <span class="font-medium text-gray-800 dark:text-gray-100">{{ $child->age ?? '-' }}</span>
                        </div>
                        @if ($child->place_of_birth)
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-gray-400">Tempat Lahir</span>
                                <span class="font-medium text-gray-800 dark:text-gray-100">{{ $child->place_of_birth }}</span>
                            </div>
                        @endif
                        @if ($child->blood_type)
                            <div class="flex justify-between">
                                <span class="text-gray-500 dark:text-gray-400">Golongan Darah</span>
                                <span class="font-medium text-gray-800 dark:text-gray-100">{{ $child->blood_type }}</span>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-3xl p-6">
                ...
                <div class="flex flex-wrap gap-3">
                    <a href="{{ route('export.profile', $child) }}" class="inline-flex items-center px-4 py-2 bg-softPink-100 hover:bg-softPink-200 dark:bg-softPink-950/30 dark:hover:bg-softPink-900/40 text-softPink-700 dark:text-softPink-400 font-medium rounded-xl text-sm transition-all duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        {{ __('Profil Anak') }}
                    </a>
                    <a href="{{ route('export.health', $child) }}" class="inline-flex items-center px-4 py-2 bg-mintGreen-100 hover:bg-mintGreen-200 dark:bg-mintGreen-950/30 dark:hover:bg-mintGreen-900/40 text-mintGreen-700 dark:text-mintGreen-400 font-medium rounded-xl text-sm transition-all duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                        </svg>
                        {{ __('Riwayat Kesehatan') }}
                    </a>
                    <a href="{{ route('export.growth', $child) }}" class="inline-flex items-center px-4 py-2 bg-skyBlue-100 hover:bg-skyBlue-200 dark:bg-skyBlue-950/30 dark:hover:bg-skyBlue-900/40 text-skyBlue-700 dark:text-skyBlue-400 font-medium rounded-xl text-sm transition-all duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                        </svg>
                        {{ __('Riwayat Pertumbuhan') }}
                    </a>
                    <a href="{{ route('export.zip', $child) }}" class="inline-flex items-center px-4 py-2 bg-warmYellow-100 hover:bg-warmYellow-200 dark:bg-warmYellow-950/30 dark:hover:bg-warmYellow-900/40 text-warmYellow-700 dark:text-warmYellow-400 font-medium rounded-xl text-sm transition-all duration-200">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
