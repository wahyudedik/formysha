        ...
        <div class="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            @if(! $notification->is_read)
                <form method="POST" action="{{ route('notifications.markAsRead', $notification) }}">
                    @csrf
                    <button type="submit" class="p-1.5 text-gray-400 hover:text-skyBlue hover:bg-skyBlue/10 dark:text-gray-500 dark:hover:text-skyBlue-400 dark:hover:bg-skyBlue/10 rounded-lg transition-colors" title="Tandai sudah dibaca">
            ...
            <form method="POST" action="{{ route('notifications.destroy', $notification) }}" x-data>
                @csrf
                @method('DELETE')
                <button type="submit" class="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:text-gray-500 dark:hover:text-red-400 dark:hover:bg-red-950/30 rounded-lg transition-colors" title="Hapus notifikasi"
