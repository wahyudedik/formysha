        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-3xl">
                <div class="p-4 sm:p-6 lg:p-8">
                    ...
                    <form method="POST" action="{{ route('documents.update', [$child, $document]) }}" enctype="multipart/form-data" x-data="{ loading: false }" @submit="loading = true">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-5">
                            ...
                            <select id="type" name="type" class="mt-1 block w-full border-gray-300 dark:border-gray-600 focus:border-skyBlue-500 focus:ring-skyBlue-500 rounded-2xl shadow-soft dark:bg-gray-700 dark:text-gray-200" required>
                                ...
                                <option value="kia" {{ old('type', $document->type) === 'kia' ? 'selected' : '' }}>🪪 KIA</option>
                                <option value="bpjs" {{ old('type', $document->type) === 'bpjs' ? 'selected' : '' }}>🏥 BPJS</option>
                                <option value="passport" {{ old('type', $document->type) === 'passport' ? 'selected' : '' }}>✈️ Paspor</option>
                                <option value="certificate" {{ old('type', $document->type) === 'certificate' ? 'selected' : '' }}>🎓 Sertifikat</option>
                                <option value="report_card" {{ old('type', $document->type) === 'report_card' ? 'selected' : '' }}>📋 Rapor</option>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                            <div>
                                ...
                                <x-text-input id="issued_date" name="issued_date" type="date" class="mt-1 block w-full" :value="old('issued_date', $document->issued_date instanceof \Carbon\Carbon ? $document->issued_date->format('Y-m-d') : $document->issued_date)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="expiry_date" name="expiry_date" type="date" class="mt-1 block w-full" :value="old('expiry_date', $document->expiry_date instanceof \Carbon\Carbon ? $document->expiry_date->format('Y-m-d') : $document->expiry_date)" />
                        ...
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                            <a href="{{ route('documents.show', [$child, $document]) }}" class="btn-secondary min-h-[44px]">
                                {{ __('Batal') }}
                            </a>
                            <button type="submit" :disabled="loading" class="btn-primary min-h-[44px] disabled:opacity-50 disabled:cursor-not-allowed">
                                ...
                                <span x-text="loading ? '{{ __('Menyimpan...') }}' : '{{ __('Perbarui Dokumen') }}'"></span>
                            </button>
                        </div>
                    </form>
                </div>
