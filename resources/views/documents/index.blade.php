<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if ($documents->isEmpty() && !$request->has('type'))
                <!-- Empty State -->
                <div class="text-center py-16">
                    <div class="w-24 h-24 mx-auto mb-6 rounded-3xl bg-gradient-to-br from-skyBlue-50 to-lavender-50 dark:from-skyBlue-950/30 dark:to-lavender-950/30 flex items-center justify-center">
                        <span class="text-5xl">📋</span>
                    </div>
                    <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-2">{{ __('Belum Ada Dokumen') }}</h3>
                    <p class="text-gray-500 dark:text-gray-400 mb-6 max-w-md mx-auto">{{ __('Simpan dokumen penting ' . ($child->nickname ?? $child->name) . ' seperti akta lahir, KK, KIA, dan lainnya di sini.') }}</p>
            ...
            @else
                ...
                <div class="mb-6 flex flex-col sm:flex-row gap-3">
                    <div class="flex items-center gap-2 flex-wrap">
                        <!-- Type Filter -->
                        <select onchange="window.location.href=this.value" class="px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-300 min-h-[44px] focus:ring-2 focus:ring-skyBlue-300 dark:focus:ring-skyBlue-600 focus:border-skyBlue-400 dark:focus:border-skyBlue-500">
                            <option value="{{ route('documents.index', array_merge(['child' => $child], ['sort' => $currentSort] + request()->except(['type', 'page']))) }}" {{ !request('type') ? 'selected' : '' }}>📋 {{ __('Semua Tipe') }}</option>
                            @foreach ($documentTypes as $typeKey => $typeLabel)
                                <option value="{{ route('documents.index', array_merge(['child' => $child], ['type' => $typeKey, 'sort' => $currentSort] + request()->except(['type', 'page']))) }}" {{ request('type') === $typeKey ? 'selected' : '' }}>{{ $typeLabel }}</option>
                            @endforeach
                        </select>

                        <!-- Sort -->
                        <select onchange="window.location.href=this.value" class="px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-300 min-h-[44px] focus:ring-2 focus:ring-skyBlue-300 dark:focus:ring-skyBlue-600 focus:border-skyBlue-400 dark:focus:border-skyBlue-500">
                            <option value="{{ route('documents.index', array_merge(['child' => $child], ['sort' => 'default'] + request()->except(['sort', 'page']))) }}" {{ $currentSort === 'default' ? 'selected' : '' }}>⭐ {{ __('Default') }}</option>
                            <option value="{{ route('documents.index', array_merge(['child' => $child], ['sort' => 'newest'] + request()->except(['sort', 'page']))) }}" {{ $currentSort === 'newest' ? 'selected' : '' }}>🕐 {{ __('Terbaru') }}</option>
                            <option value="{{ route('documents.index', array_merge(['child' => $child], ['sort' => 'oldest'] + request()->except(['sort', 'page']))) }}" {{ $currentSort === 'oldest' ? 'selected' : '' }}>🕐 {{ __('Terlama') }}</option>
                            <option value="{{ route('documents.index', array_merge(['child' => $child], ['sort' => 'name_asc'] + request()->except(['sort', 'page']))) }}" {{ $currentSort === 'name_asc' ? 'selected' : '' }}>🔤 {{ __('Nama A-Z') }}</option>
                            <option value="{{ route('documents.index', array_merge(['child' => $child], ['sort' => 'name_desc'] + request()->except(['sort', 'page']))) }}" {{ $currentSort === 'name_desc' ? 'selected' : '' }}>🔤 {{ __('Nama Z-A') }}</option>
                        </select>
                    </div>

                    @if ($documents->hasPages() || $documents->total() > 0)
                        <div class="text-sm text-gray-400 dark:text-gray-500 flex items-center">
                            {{ $documents->total() }} {{ __('dokumen') }}
                            @if (request('type'))
                                <a href="{{ route('documents.index', ['child' => $child, 'sort' => $currentSort]) }}" class="ml-2 text-skyBlue-500 hover:text-skyBlue-600 dark:text-skyBlue-400 dark:hover:text-skyBlue-300 underline">
                                    {{ __('Reset filter') }}
                                </a>
                            @endif
                        </div>
