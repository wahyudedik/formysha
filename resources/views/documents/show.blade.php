                        ...
                        <div x-data="{ showDeleteConfirm: false }">
                            ...
                            <div x-show="showDeleteConfirm" x-cloak class="fixed inset-0 z-50 overflow-y-auto" x-transition>
                                <div class="flex items-center justify-center min-h-screen px-4">
                                    <div class="fixed inset-0 bg-gray-900/50 dark:bg-gray-950/75" @click="showDeleteConfirm = false"></div>
                                    <div class="relative bg-white dark:bg-gray-800 rounded-3xl shadow-soft max-w-md w-full p-6 z-10">
                                        <h3 class="text-lg font-bold text-gray-800 dark:text-gray-100 mb-2">{{ __('Hapus Dokumen?') }}</h3>
                                        ...
                                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 justify-end">
                                            <button @click="showDeleteConfirm = false" class="btn-secondary min-h-[44px]">{{ __('Batal') }}</button>
                                            <form method="POST" action="{{ route('documents.destroy', [$child, $document]) }}" x-on:submit="showDeleteConfirm = false">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="inline-flex items-center justify-center px-4 py-2 bg-red-500 text-white font-semibold text-sm rounded-xl hover:bg-red-600 transition min-h-[44px]">{{ __('Hapus') }}</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
