        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-3xl">
                <div class="p-4 sm:p-6 lg:p-8">
                    ...
                    <form method="POST" action="{{ route('growth.store', $child) }}" x-data="{ loading: false }" @submit="loading = true">
                        @csrf

                        <!-- Date -->
                        <div class="mb-5">
                            <x-input-label for="measured_at" :value="__('Tanggal Pengukuran')" />
                            <x-text-input id="measured_at" name="measured_at" type="date" class="mt-1 block w-full" :value="old('measured_at', date('Y-m-d'))" required autofocus />
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                            <div>
                                ...
                                <x-text-input id="weight_kg" name="weight_kg" type="number" step="0.1" min="0.1" max="200" class="mt-1 block w-full" :value="old('weight_kg')" placeholder="Contoh: 12.5" />
                                <x-input-error :messages="$errors->get('weight_kg')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="height_cm" :value="__('Tinggi Badan (cm)')" />
                                <x-text-input id="height_cm" name="height_cm" type="number" step="0.1" min="1" max="250" class="mt-1 block w-full" :value="old('height_cm')" placeholder="Contoh: 85.0" />
                        ...
                        <div class="mb-5">
                            ...
                            <x-text-input id="head_circumference_cm" name="head_circumference_cm" type="number" step="0.1" min="1" max="100" class="mt-1 block w-full" :value="old('head_circumference_cm')" placeholder="Contoh: 45.0" />
                        ...
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                            <a href="{{ route('growth.index', $child) }}" class="btn-secondary min-h-[44px]">
                                {{ __('Batal') }}
                            </a>
                            <button type="submit" :disabled="loading" class="btn-primary min-h-[44px] disabled:opacity-50 disabled:cursor-not-allowed">
                                ...
                                <span x-text="loading ? '{{ __('Menyimpan...') }}' : '{{ __('Simpan Pengukuran') }}'"></span>
                            </button>
                        </div>
                    </form>
                </div>
