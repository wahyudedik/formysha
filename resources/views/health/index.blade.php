<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @if (session('status'))
                <div class="mb-6 p-4 bg-mintGreen-50 border border-mintGreen-200 text-mintGreen-700 rounded-xl text-sm">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <div class="mb-6 flex flex-wrap gap-2">
                <a href="{{ route('health.index', $child) }}"
                   class="px-4 py-2 rounded-xl text-sm font-medium transition {{ !$activeType ? 'bg-skyBlue-500 text-white' : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200' }}">
                    Semua
                </a>
                @php
                    $types = [
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="divide-y divide-gray-50">
                        @foreach ($healthRecords as $record)
                            <a href="{{ route('health.show', [$child, $record]) }}" class="flex items-start gap-4 p-5 hover:bg-gray-50 transition">
                                <div class="flex-shrink-0 w-12 h-12 rounded-xl {{ $record->type_color }} flex items-center justify-center text-xl">
                                    {{ $record->type_icon }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-center gap-2 mb-1">
                                        <h3 class="text-sm font-semibold text-gray-800 truncate">{{ $record->name }}</h3>
                                    ...
                                    @if ($record->description)
                                        <p class="text-sm text-gray-600 mt-1 line-clamp-1">{{ $record->description }}</p>
                                    @endif
                                    ...
                                    @if ($record->next_date)
                                        <div class="mt-1 text-xs text-warmYellow-600 font-medium">
                                            📅 Jadwal berikutnya: {{ $record->formatted_next_date }}
                                        </div>
                                    @endif
                                </div>
                                <div class="flex-shrink-0 flex items-center gap-2">
                                    <a href="{{ route('health.edit', [$child, $record]) }}" class="text-skyBlue-600 hover:text-skyBlue-700 text-xs font-medium">
                                        ✏️ Edit
                                    </a>
                                    <form method="POST" action="{{ route('health.destroy', [$child, $record]) }}" x-data>
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button type="submit" class="text-red-500 hover:text-red-600 text-xs font-medium" x-data x-on:click.prevent="if(confirm('Yakin ingin menghapus catatan kesehatan ini?')) $el.closest('form').submit()">
                                            🗑️ Hapus
                                        </button>
                                    </form>
                                </div>
