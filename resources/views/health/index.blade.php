    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
            ...
            @else
                        ...
                        @foreach ($healthRecords as $record)
                            <a href="{{ route('health.show', [$child, $record]) }}" class="flex items-start gap-3 sm:gap-4 p-4 sm:p-5 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                                ...
                                <div class="shrink-0 flex items-center gap-1">
                                    ...
                                    <button type="button" class="inline-flex items-center justify-center w-9 h-9 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition" title="Hapus"
                                        x-data
                                        x-on:click.prevent="$dispatch('delete-confirm', 'delete-health-{{ $record->id }}')">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @foreach ($healthRecords as $record)
                    <x-confirm-delete
                        id="delete-health-{{ $record->id }}"
                        title="{{ __('Hapus Catatan Kesehatan') }}"
                        message="{{ __('Apakah Anda yakin ingin menghapus catatan kesehatan ini? Tindakan ini tidak dapat dibatalkan.') }}"
                        action="{{ route('health.destroy', [$child, $record]) }}"
                    />
                @endforeach
