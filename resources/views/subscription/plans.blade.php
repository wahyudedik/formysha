<x-app-layout>
    ...
    <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="text-center mb-10">
            <h1 class="text-3xl font-bold text-gray-800 mb-2">Pilih Paket Terbaik</h1>
            <p class="text-gray-500 max-w-lg mx-auto">Sesuaikan kebutuhan keluarga Anda dengan paket yang tepat. Mulai dari gratis hingga enterprise.</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach ($plans as $plan)
                ...
                <div class="relative bg-gradient-to-br {{ $gradient }} rounded-3xl border {{ $borderColor }} overflow-hidden transition-all duration-300 hover:shadow-soft-lg hover:-translate-y-1">
                    ...
                    <div class="p-6">
                        ...
                        @else
                            <p class="text-xs text-gray-500 mb-4">&nbsp;</p>
                        @endif
                        ...
                        <div class="mb-6">
                            @if ($isFree)
                                <div class="flex items-baseline gap-1">
                                    <span class="text-3xl font-bold text-gray-800">Gratis</span>
                                </div>
                                <p class="text-xs text-gray-500 mt-1">Selamanya</p>
                            @else
                                <div class="flex items-baseline gap-1">
                                    <span class="text-xs text-gray-500">Rp</span>
                                    <span class="text-3xl font-bold text-gray-800">{{ number_format($plan->price_monthly, 0, ',', '.') }}</span>
                                </div>
                        ...
                        @if ($plan->features && count($plan->features) > 0)
                            <div class="space-y-2 mb-6 pt-4 border-t border-gray-200/50">
        ...
        <div class="mt-16 max-w-2xl mx-auto">
            <h2 class="text-xl font-bold text-gray-800 text-center mb-6">❓ Pertanyaan Umum</h2>
            <div class="space-y-4">
                <div class="bg-white rounded-2xl shadow-soft p-5" x-data="{ open: false }">
                    <button @click="open = !open" class="w-full flex items-center justify-between text-left">
                        <span class="text-sm font-semibold text-gray-800">Bagaimana cara membayar?</span>
                        <svg class="w-5 h-5 text-gray-400 transition-transform" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    <div x-show="open" x-collapse class="mt-3 text-sm text-gray-500">
                ...
                <div class="bg-white rounded-2xl shadow-soft p-5" x-data="{ open: false }">
                    <button @click="open = !open" class="w-full flex items-center justify-between text-left">
                        <span class="text-sm font-semibold text-gray-800">Bisa ganti paket kapan saja?</span>
                        <svg class="w-5 h-5 text-gray-400 transition-transform" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                ...
                <div class="bg-white rounded-2xl shadow-soft p-5" x-data="{ open: false }">
                    <button @click="open = !open" class="w-full flex items-center justify-between text-left">
                        <span class="text-sm font-semibold text-gray-800">Apakah data saya aman?</span>
                        <svg class="w-5 h-5 text-gray-400 transition-transform" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
