...
<body class="gradient-bg min-h-screen">
    <div class="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="card p-8 mb-8 text-center fade-in">
            ...
            <div class="mb-6">
                @if($child->photo)
                    <img src="{{ asset('storage/'.$child->photo) }}"
                         ...
                         class="w-32 h-32 rounded-full object-cover mx-auto border-4 border-white shadow-soft">
                @else
                    <div class="w-32 h-32 rounded-full mx-auto border-4 border-white shadow-soft bg-gradient-to-br from-softPink-200 to-lavender-200 flex items-center justify-center">
                        <span class="text-5xl">
                            {{ $child->gender === 'female' ? '👧' : '👦' }}
                        </span>
                    </div>
            ...
            @if($child->nickname)
                <p class="text-gray-500 mb-3">{{ $child->name }}</p>
            @endif
            ...
            <div class="flex flex-wrap justify-center gap-3 mt-4">
                @if($child->age)
                    <span class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-warmYellow-100 text-warmYellow-500 text-sm font-medium">
                        🎂 {{ $child->age }}
                    </span>
                @endif
                @if($child->place_of_birth)
                    <span class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-mintGreen-100 text-mintGreen-500 text-sm font-medium">
                        📍 {{ $child->place_of_birth }}
                    </span>
                @endif
                @if($child->gender)
                    <span class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-softPink-100 text-softPink-500 text-sm font-medium">
                        {{ $child->gender === 'female' ? '👧 Perempuan' : '👦 Laki-laki' }}
                    </span>
                @endif
                @if($child->blood_type)
                    <span class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-softPink-100 text-red-500 text-sm font-medium">
            ...
            <div class="card p-6 mb-8 fade-in" style="animation-delay: 0.1s;">
                ...
                <div class="space-y-4">
                    @foreach($child->timelines as $timeline)
                        <div class="flex gap-4 p-4 rounded-xl bg-gradient-to-r from-lavender-50 to-softPink-50">
                            <div class="flex-shrink-0">
                                <div class="w-10 h-10 rounded-full bg-lavender-200 flex items-center justify-center text-sm">
                            ...
                            <div class="flex-1 min-w-0">
                                <h3 class="font-semibold text-gray-800">{{ $timeline->title }}</h3>
                                @if($timeline->description)
                                    <p class="text-gray-600 text-sm mt-1 line-clamp-2">{{ $timeline->description }}</p>
                                @endif
                            </div>
                        </div>
                    @endforeach
            ...
            <div class="card p-6 mb-8 fade-in" style="animation-delay: 0.2s;">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    @forelse($child->albums->take(6) as $album)
                        <div class="aspect-square rounded-xl overflow-hidden bg-gradient-to-br from-peach-100 to-warmYellow-100 flex items-center justify-center">
