            ...
            @else
                ...
                <div class="space-y-4">
                    @foreach ($familyMembers as $member)
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-2xl p-6 flex items-center gap-4">
                            <!-- Avatar -->
                            <div class="shrink-0">
                                @if ($member->photo)
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                <a href="{{ route('family.edit', [$child, $member]) }}" class="p-2 text-gray-400 hover:text-softPink-500 hover:bg-softPink-50 dark:text-gray-500 dark:hover:text-softPink-400 dark:hover:bg-softPink-950/20 rounded-xl transition">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <form method="POST" action="{{ route('family.destroy', [$child, $member]) }}" x-data="{ confirming: false }" @submit.prevent="if(confirm('Apakah Anda yakin ingin menghapus {{ addslashes($member->name) }}?')) $el.submit();">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:text-gray-500 dark:hover:text-red-400 dark:hover:bg-red-950/30 rounded-xl transition">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
