<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('connections.show', [$child, $connection]) }}" class="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 transition">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-3xl p-4 sm:p-6 lg:p-8">
                <div class="mb-6">
                    <p class="text-sm text-gray-500 dark:text-gray-400">
                        {{ __('Mengedit koneksi dengan') }} <span class="font-semibold text-gray-800 dark:text-gray-100">{{ $connection->tenant->name ?? '-' }}</span>
                    </p>
                </div>

                <form method="POST" action="{{ route('connections.update', [$child, $connection]) }}" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="permission" name="permission" class="mt-1 block w-full border-gray-300 focus:border-softPink-300 focus:ring-softPink-200 rounded-xl shadow-sm transition dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200" required>
                            @foreach ($permissions as $permission)
                                <option value="{{ $permission->value }}" {{ old('permission', $connection->permission->value) === $permission->value ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full border-gray-300 focus:border-softPink-300 focus:ring-softPink-200 rounded-xl shadow-sm transition dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200" placeholder="{{ __('Catatan opsional tentang koneksi ini...') }}">{{ old('notes', $connection->notes) }}</textarea>
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-4">
                        ...
                        <a href="{{ route('connections.show', [$child, $connection]) }}" class="btn-secondary min-h-[44px]">
                            {{ __('Batal') }}
                        </a>
                    </div>
                </form>
