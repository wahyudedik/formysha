        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2">
                        <div class="bg-white rounded-2xl shadow-soft p-6 border border-gray-100">
                            <div class="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    <h3 class="font-bold text-gray-800 text-xl">{{ $plugin->name }}</h3>
                            ...
                            <div class="mb-4">
                                ...
                                <p class="text-sm text-gray-800">{{ $plugin->description ?? 'Tidak ada deskripsi.' }}</p>
                            </div>

                            @if($plugin->hooks)
                                <div class="mb-4">
                                    <h4 class="text-sm font-medium text-gray-500 mb-2">Hooks</h4>
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($plugin->hooks as $hook)
                                            <span class="px-2.5 py-1 rounded-full text-xs font-medium bg-lavender-100 text-lavender-700">{{ $hook }}</span>
                                        @endforeach
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($plugin->permissions)
                                <div class="mb-4">
                                    <h4 class="text-sm font-medium text-gray-500 mb-2">Permissions</h4>
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($plugin->permissions as $permission)
                                            <span class="px-2.5 py-1 rounded-full text-xs font-medium bg-warmYellow-100 text-warmYellow-700">{{ $permission }}</span>
                                        @endforeach
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="bg-white rounded-2xl shadow-soft p-6 border border-gray-100 mt-6">
                            <h4 class="font-bold text-gray-800 mb-4">📋 Log Aktivitas Terbaru</h4>
                                ...
                                <div class="space-y-3">
                                    @foreach($recentLogs as $log)
                                        <div class="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm text-gray-800">{{ $log->message ?? $log->action }}</p>
                    ...
                    <div>
                        <div class="bg-white rounded-2xl shadow-soft p-6 border border-gray-100">
                            <h4 class="font-bold text-gray-800 mb-4">📊 Statistik</h4>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-600">Resmi</span>
                                    <span class="text-sm font-medium text-gray-800">{{ $plugin->is_official ? '✅ Ya' : '❌ Tidak' }}</span>
                                </div>

                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-600">Install</span>
                                    <span class="text-sm font-medium text-gray-800">{{ $plugin->install_count }}</span>
                                </div>

                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-600">Tenant Plugins</span>
                                    <span class="text-sm font-medium text-gray-800">{{ $plugin->tenant_plugins_count }}</span>
                                </div>

                                <div class="flex items-center justify-between">
                                    <span class="text-sm text-gray-600">Dibuat</span>
                                    <span class="text-sm text-gray-800">{{ $plugin->created_at->format('d M Y') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
