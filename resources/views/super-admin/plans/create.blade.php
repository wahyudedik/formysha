    ...
    <div class="py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <form method="POST" action="{{ route('super-admin.plans.store') }}" x-data="{ loading: false }" @submit="loading = true">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-soft overflow-hidden">
                            ...
                            <div class="p-4 sm:p-6 space-y-5">
                                ...
                                <div>
                                    <x-input-label for="slug" :value="__('plans.slug_optional')" />
                                    <x-text-input id="slug" name="slug" type="text" class="mt-1 block w-full input-focus" :value="old('slug')" placeholder="{{ __('plans.slug_auto_hint') }}" />
                                    <x-input-error :messages="$errors->get('slug')" class="mt-1" />
                                </div>

                                <div>
                                    <x-input-label for="description" :value="__('forms.description')" />
                                    <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 rounded-xl focus:border-softPink-300 focus:ring-softPink-200 text-sm" placeholder="{{ __('plans.description_placeholder') }}">{{ old('description') }}</textarea>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-soft overflow-hidden">
                            ...
                            <div class="p-4 sm:p-6 space-y-5">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ __('plans.free_hint') }}</p>
                                </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-soft overflow-hidden">
                            ...
                            <div class="p-4 sm:p-6">
                                ...
                                <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ __('plans.features_desc') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <button type="submit" :disabled="loading" class="btn-primary text-sm min-h-[44px] inline-flex items-center disabled:opacity-50 disabled:cursor-not-allowed">
                            ...
                            <span x-text="loading ? '{{ __('actions.saving') }}' : '{{ __('plans.save_plan') }}'"></span>
                        </button>
