<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('timeline.show', [$child, $timeline]) }}" class="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 transition">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-soft sm:rounded-3xl">
                <div class="p-4 sm:p-6 lg:p-8">
                    ...
                    <div class="flex items-center gap-3 mb-6 p-4 bg-gradient-to-r from-lavender-50 to-softPink-50 dark:from-lavender-950/30 dark:to-softPink-950/30 rounded-2xl">
                        ...
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Mengedit kenangan untuk') }}</p>
                    ...
                    <form method="POST" action="{{ route('timeline.update', [$child, $timeline]) }}" x-data="{ loading: false }" @submit="loading = true">
                        ...
                        <div class="mb-5">
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" :value="old('title', $timeline->title)" required autofocus />
                        ...
                        <div class="mb-5">
                            ...
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-lavender-500 focus:ring-lavender-500 rounded-2xl shadow-soft">{{ old('description', $timeline->description) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                            <div>
                                ...
                                <x-text-input id="event_date" name="event_date" type="date" class="mt-1 block w-full" :value="old('event_date', $timeline->event_date->format('Y-m-d'))" required />
                                <x-input-error :messages="$errors->get('event_date')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="event_time" :value="__('Waktu (opsional)')" />
                                <x-text-input id="event_time" name="event_time" type="time" class="mt-1 block w-full" :value="old('event_time', $timeline->event_time)" />
                        ...
                        <div class="mb-5">
                            ...
                            <x-text-input id="location" name="location" type="text" class="mt-1 block w-full" :value="old('location', $timeline->location)" />
                        ...
                        <div class="mb-5">
                            ...
                            <select id="mood" name="mood" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-lavender-500 focus:ring-lavender-500 rounded-2xl shadow-soft">
                                ...
                                <option value="happy" {{ old('mood', $timeline->mood) === 'happy' ? 'selected' : '' }}>😊 Bahagia</option>
                                <option value="excited" {{ old('mood', $timeline->mood) === 'excited' ? 'selected' : '' }}>🤩 Antusias</option>
                                <option value="calm" {{ old('mood', $timeline->mood) === 'calm' ? 'selected' : '' }}>😌 Tenang</option>
                                <option value="sad" {{ old('mood', $timeline->mood) === 'sad' ? 'selected' : '' }}>😢 Sedih</option>
                                <option value="surprised" {{ old('mood', $timeline->mood) === 'surprised' ? 'selected' : '' }}>😲 Terkejut</option>
                                <option value="loved" {{ old('mood', $timeline->mood) === 'loved' ? 'selected' : '' }}>🥰 Disayang</option>
                            </select>
                        ...
                        <div class="mb-5">
                            ...
                            <x-text-input id="tags" name="tags" type="text" class="mt-1 block w-full" :value="old('tags', is_array($timeline->tags) ? implode(', ', $timeline->tags) : '')" placeholder="Pisahkan dengan koma: milestone, keluarga, sekolah" />
                            <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">{{ __('Pisahkan setiap tag dengan koma') }}</p>
                            <x-input-error :messages="$errors->get('tags')" class="mt-2" />
                        </div>

                        <!-- Featured -->
                        <div class="mb-6" x-data="{ checked: {{ old('is_featured', $timeline->is_featured) ? 'true' : 'false' }} }">
