...
<body class="bg-gradient-to-br from-pink-50 via-white to-purple-50 min-h-screen text-gray-700">
    <header class="w-full px-4 sm:px-6 lg:px-8">
        <nav class="max-w-4xl mx-auto flex items-center justify-between py-4 sm:py-6">
            <a href="{{ url('/') }}" class="flex items-center gap-2 no-underline">
                ...
                <span class="font-extrabold text-lg sm:text-xl text-gray-800">ForMysha</span>
            </a>
            <a href="{{ url('/') }}" class="hidden sm:inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 transition-colors no-underline">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
                Kembali
            </a>
            <a href="{{ url('/') }}" class="sm:hidden flex items-center justify-center w-10 h-10 rounded-xl hover:bg-white/60 transition-colors no-underline" aria-label="Kembali ke Beranda">
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20">
        <div class="bg-white rounded-3xl shadow-soft p-6 sm:p-10 lg:p-12">
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-6">{{ $pageTitle }}</h1>
            {{ $slot }}
        </div>
    </main>
