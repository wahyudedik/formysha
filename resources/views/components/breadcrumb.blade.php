...
@if ($breadcrumbItems->isNotEmpty())
    <nav class="flex items-center text-sm text-gray-400 mb-4 overflow-x-auto" aria-label="Breadcrumb">
        <ol class="flex items-center gap-1.5 whitespace-nowrap">
            @foreach ($breadcrumbItems as $index => $item)
                ...
                @if (isset($item['url']) && $index !== $breadcrumbItems->count() - 1)
                    <a href="{{ $item['url'] }}" class="hover:text-gray-600 transition font-medium">
                        {{ $item['label'] }}
                    </a>
                @else
