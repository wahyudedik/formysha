...
>
    ...
    <div
        ...
        :class="isDragging ? 'border-lavender-400 bg-lavender-50' : 'border-gray-300 hover:border-lavender-400 bg-gradient-to-br from-lavender-50/50 to-softPink-50/50'"
    ...
    <template x-if="existingMedia.length > 0">
        <div class="mt-3 space-y-2">
            <p class="text-xs text-gray-400 font-medium">{{ __('Media yang sudah ada:') }}</p>
            <template x-for="(media, index) in existingMedia" :key="media.id || index">
                <div class="flex items-center gap-3 p-3 bg-mintGreen-50 border border-mintGreen-200 rounded-xl">
                    <span class="text-2xl" x-text="media.type === 'photo' ? '🖼️' : media.type === 'video' ? '🎬' : media.type === 'audio' ? '🎵' : '📄'"></span>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-medium text-gray-700 truncate" x-text="media.name"></p>
                        <p class="text-xs text-gray-400" x-text="media.size"></p>
                    </div>
                </div>
            </template>
        </div>
