...
describe('Album API', function () {
    it('can list albums', function () {
        ...
        $response = $this->withHeader('Authorization', 'Bearer '.$token)
            ->getJson('/api/v1/children/'.$child->slug.'/albums');
    ...
    it('can create an album', function () {
        ...
        $response = $this->withHeader('Authorization', 'Bearer '.$token)
            ->postJson('/api/v1/children/'.$child->slug.'/albums', [
                'name' => 'Album Keluarga',
            ]);
    ...
    it('can show album with media', function () {
        ...
        $response = $this->withHeader('Authorization', 'Bearer '.$token)
            ->getJson('/api/v1/children/'.$child->slug.'/albums/'.$album->id);
    ...
    it('can update an album', function () {
        ...
        $response = $this->withHeader('Authorization', 'Bearer '.$token)
            ->putJson('/api/v1/children/'.$child->slug.'/albums/'.$album->id, [
                'name' => 'Updated Album',
            ]);
    ...
    it('can delete an album', function () {
        ...
        $response = $this->withHeader('Authorization', 'Bearer '.$token)
            ->deleteJson('/api/v1/children/'.$child->slug.'/albums/'.$album->id);
